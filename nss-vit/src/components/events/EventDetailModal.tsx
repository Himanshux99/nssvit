      ...
      {selectedEvent && (
        <div className="fixed inset-0 z-[10000] flex items-start justify-center p-3 pt-16 sm:p-4 sm:pt-20 overflow-y-auto">
          ...
          >
            ...
            {/* Sticky Report Footer — only for completed events with a report link */}
            {selectedEvent.status === "completed" && selectedEvent.reportLink && (
              <div className="flex-shrink-0 border-t border-swiss-gray-200 bg-gradient-to-r from-swiss-gray-50 to-white px-4 sm:px-6 py-3.5">
                <a
                  href={selectedEvent.reportLink}
                  target="_blank"
                  rel="noopener noreferrer"
                  onClick={(e) => e.stopPropagation()}
                  className="group/report flex items-center justify-center gap-2.5 w-full py-2.5 px-5 bg-gradient-to-r from-indigo-600 to-primary text-white text-sm font-semibold rounded-md shadow-md hover:shadow-lg hover:from-indigo-700 hover:to-indigo-600 transition-all duration-200 cursor-pointer active:scale-[0.98]"
                >
                  {/* Document icon */}
                  ...
                  {/* Arrow icon */}
                  <svg
                    className="w-3.5 h-3.5 flex-shrink-0 transition-transform duration-200 group-hover/report:translate-x-0.5"
                    fill="none"
                    viewBox="0 0 24 24"
                    stroke="currentColor"
                    strokeWidth={2.5}
